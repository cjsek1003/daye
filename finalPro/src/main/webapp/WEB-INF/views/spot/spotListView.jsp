 ...
 <%--    <div align="right">
    ...
    <div>
      <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          <c:forEach var="sp" items="${spotList}" varStatus="status">
            <tr>
              <td>${sp.s_no}</td>
              <td>${sp.s_nation}</td>
              <td id="title">
<%--                  <a href="/bbs/content.bbs?articleNumber=${spot.s_no}&pageNum=${pageNum}">${spot.s_title}</a> --%>     
           <a href="/bbs/content.bbs?articleNumber=${sp.s_no}">${sp.s_title}</a>
                <c:if test="${sp.s_like_count >= 20}">
                  <span class="hit">hit!</span>
                </c:if>
              </td>
              <td>${sp.s_writer}</td>
              <td>${sp.s_like_count}</td>
            <tr>
          </c:forEach>
        </tbody>
      </table>
